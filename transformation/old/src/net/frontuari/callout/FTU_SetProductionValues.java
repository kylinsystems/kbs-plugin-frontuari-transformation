			...
			if(MProduction.COLUMNNAME_M_Product_ID.equalsIgnoreCase(mField.getColumnName())) {
				...
				if(M_Product_ID>0){
					...
					String sql = "SELECT inv.c_invoiceline_id,trans.m_productionline_id,prod.m_product_id,"
							+ " (CASE WHEN inv.dateacct=trans.movementdate THEN (CASE WHEN inv.processedon<trans.processedon THEN trans.m_production_id ELSE 0 END)"
							+ " WHEN  inv.c_invoiceline_id>0 AND trans.m_productionline_id>0 THEN (CASE WHEN inv.dateacct>trans.movementdate THEN 0 ELSE trans.m_production_id END)"
							+ " WHEN trans.m_productionline_id>0 THEN trans.m_production_id ELSE 0 END) as  m_production_id,"
							+ "(CASE  WHEN inv.dateacct=trans.movementdate THEN (CASE WHEN inv.processedon>trans.processedon THEN 0 ELSE inv.c_invoice_id END)"
							+ " WHEN  inv.c_invoiceline_id>0 AND trans.m_productionline_id>0 THEN (CASE WHEN inv.dateacct>trans.movementdate THEN inv.c_invoice_id ELSE 0 END)"
							+ " WHEN inv.c_invoiceline_id>0 THEN inv.c_invoice_id ELSE 0 END) AS c_invoice_id,"
							+ " COALESCE((CASE "
							...
							+ " LEFT JOIN ( SELECT i.dateacct,p.m_product_id,il.c_invoiceline_id, "
								...
								+ " ELSE il.priceentered end),0) ELSE 0 END as priceentered, to_timestamp(i.processedon /1000) AS processedon, i.c_invoice_id  "
							...
							+ " ) AS inv ON prod.m_product_id =inv.m_product_id LEFT JOIN ("
								+ " SELECT mpl.m_product_id ,mpl.m_productionline_id,mp.movementdate,mpl.pricelastinv, to_timestamp(mp.processedon /1000) AS processedon, mp.m_production_id "
					...
					try{
						pstmt = DB.prepareStatement(sql,null);
						 ...
						 if(rs.next()){
							 ...
							 if(productionId>0)
								 mTab.setValue("LastProduction_ID", productionId);
